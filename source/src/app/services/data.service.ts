...
export class DataService {
    ...
    protected apiServer = "https://api.cosmos.ai/";
    protected textApiServer;
    protected bingApiServer;
    
    // VS: do not check this in
    protected cosmosApiServer ;
    //protected cosmosApiServer = "http://localhost:5347/";

    constructor(protected http: Http) {        
        let url = environment.parametersUrl; 
        this.http.get(url)
        .map(res => res.json())
        .subscribe(data => {
                             let datakeys = data.env_variables;
                             this.storageUrl = datakeys.storageUrl;
                             this.localServer = datakeys.localServer;                 
                             this.textApiServer = datakeys.textApiServer;
                             this.bingApiServer = datakeys.bingApiServer;
                             this.cosmosApiServer = datakeys.cosmosApiServer;
                   });           
    }
